...
const Project: FC<IProject> = ({ title, project }) => {
  ...
  const stack = project.stack.map((item, index) => (
    <li className={s.project__stack} key={index}>
      <ProjectStack stack={item} />{" "}
    </li>
  ));
  ...
  return (
    <section className={`${s.project} ${isOpenProject && s.project_opened}`}>
      <button
        type="button"
        className={`button ${s.project__button}`}
        onClick={handleClick}
      >
        <h3 className={s.project__title}>{title}</h3>
        <div
          className={`${s.project__arrow} ${
            isOpenProject && s.project__arrow_active
          }`}
        >
          ↗
        </div>
      ...
      <div
        className={`${s.project__about} ${
          isOpenProject && s.project__about_active
        }`}
      >
        <div className={s.project__flex}>
          ...
          <div className={s.project__columns}>
            ...
            <ul className={`list ${s.project__list}`}>{stack}</ul>
          </div>
        </div>
        <div className={s.project__flexLink}>
          {project.link.length > 0 && (
            <a
              className={`link ${s.project__link}`}
              href={project.link}
              target="_blank"
              rel="noreferrer"
            >
